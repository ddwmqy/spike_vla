...
#   WEIGHT_SOURCE=ema EVAL_SUITES=libero_spatial,libero_goal bash /data/260010028/dwh_vla/eval_v3.sh
#       # D1 supplementary EMA eval (separate output dir; raw results untouched)
...
export WEIGHT_SOURCE="${WEIGHT_SOURCE:-raw}"      # default raw: anchor comparison is raw<->raw.
                                                  # v3 ckpts DO ship EMA (ema_decay 0.999, verified);
                                                  # WEIGHT_SOURCE=ema -> supplementary eval, own dir.
...
export EVAL_OUTPUT_ROOT="${EVAL_OUTPUT_ROOT:-$PKG/output/eval/v3_sootspike_step_${TRAIN_STEP}_${WEIGHT_SOURCE}_clean_8f1084e}"

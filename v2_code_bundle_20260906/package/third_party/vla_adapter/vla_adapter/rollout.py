...
def eval_libero(cfg: GenerateConfig) -> float:
    _setup_logging(cfg)
    logging.info("Evaluation adapter source: %s", Path(__file__).resolve())
    ...
    if cfg.spikinglm_source_path:
        spikinglm_model = (
            Path(cfg.spikinglm_source_path)
            / "spiking_bert"
            / "modeling_spiking_bert.py"
        )
        if not spikinglm_model.is_file():
            raise FileNotFoundError(
                "SpikingLM source not found: "
                f"{spikinglm_model}. Pass the repository root with "
                "--spikinglm_source_path."
            )
    ...
    policy = TurboVLAPolicy(
        ...
        spikinglm_source_path=cfg.spikinglm_source_path,
    ...
    summary = {
        ...
        "max_videos_per_task": cfg.max_videos_per_task,
        "dinov3_output_hidden_states": cfg.dinov3_output_hidden_states,
        "tasks": [],
        "total_episodes": 0,
        "total_successes": 0,
    ...
    for task_id in tqdm.tqdm(task_ids, desc="tasks"):
        ...
        try:
            for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task), desc=f"task {task_id}", leave=False):
                ...
                if cfg.save_video and (
                    cfg.max_videos_per_task < 0 or episode_idx < cfg.max_videos_per_task
                ):

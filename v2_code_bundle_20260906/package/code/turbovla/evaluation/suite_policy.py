...
class TurboVLAPolicy(base.TurboVLAPolicy):
    def __init__(
        ...
        **kwargs,
        ...
        # train1.sh enters through train_mixed.py, which replaces the base RLDS
        # dataset with LiberoMixedRLDSDataset. That dataset reads this exact JSON
        # entry, so evaluation must use it as well.
        ...
        self.stats_source = "mixed_suite_json"
        ...
        self._logged_first_env_action = False
        ...
        if self.verbose:
            print(
                ...
                f"source={self.stats_source}, "
                f"metadata_path={self.stats_path}, metadata_key={self.stats_key}, "
    ...
    def predict_env_action_chunk(
        ...
        if self.verbose and not self._logged_first_env_action:
            self._logged_first_env_action = True
            print(
                "[TurboVLAPolicy] first denormalized env action: "
                f"min={env_actions.min():.6f}, max={env_actions.max():.6f}, "
                f"mean={env_actions.mean():.6f}, std={env_actions.std():.6f}, "
                f"first_action={env_actions[0].tolist()}",
                flush=True,
            )

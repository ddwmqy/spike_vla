...
def _build_manual_rgb_normalizer(
    ...
    def process_one(img: Image.Image | np.ndarray) -> torch.Tensor:
        ...
        if resize_input and (height != expected_size or width != expected_size):
            img = img.resize((expected_size, expected_size), Image.Resampling.BICUBIC)
            width, height = img.size
...
def build_sdtv3_manual_processor():
    return _build_manual_rgb_normalizer(
...
class TurboVLAPolicy:
    def __init__(
        ...
        self._logged_first_prediction = False
        ...
        if model_config is not None:
            ...
            if config.text.encoder_type == "spikinglm" and spikinglm_source_path:
                config.text.model_source_path = spikinglm_source_path
            ...
            config.vision.compute_precision = "bf16" if self.precision == "bf16" else "fp32"
            self.vision_encoder_type = config.vision.encoder_type
            ...
            self.vision_encoder_type = "dinov3"
        ...
        self.vision_processor = (
            build_sdtv3_manual_processor()
            if self.vision_encoder_type == "sdtv3_19m"
            else build_dinov3_manual_processor(self.dinov3_path)
        )
        self.dinov3_processor = self.vision_processor
    ...
    def reset_spiking_state(self) -> None:
        """Reset policy membrane state at an episode boundary when using manual reset."""
        reset = getattr(self.model, "reset_spiking_state", None)
    ...
    def _build_batch(
        ...
        pixel_values = self.vision_processor(flat_images)["pixel_values"]
        batch_size = len(primary_images)
        samples = {
            "pixel_values": pixel_values.view(batch_size, 2, *pixel_values.shape[1:]).to(self.device),
        }
    ...
    def predict_normalized_action_chunk(
        ...
        pred_chunk = sanitize_pred_chunk(pred.detach().float().cpu().numpy()[0])
        if self.verbose and not self._logged_first_prediction:
            self._logged_first_prediction = True
            print(
                "[TurboVLAPolicy] first normalized prediction: "
                f"shape={pred_chunk.shape}, min={pred_chunk.min():.6f}, "
                f"max={pred_chunk.max():.6f}, mean={pred_chunk.mean():.6f}, "
                f"std={pred_chunk.std():.6f}, first_action={pred_chunk[0].tolist()}",
                flush=True,
            )
        return pred_chunk
    ...
    def predict_env_action_chunk(
        ...
        pred_norm = self.predict_normalized_action_chunk(
            primary_image,
            wrist_image,
            instruction,
            state_or_obs,

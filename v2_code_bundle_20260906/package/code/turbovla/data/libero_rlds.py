...
class LiberoRLDSDataset(IterableDataset):
    def __init__(
        ...
        self.vision_encoder_type = str(vision_encoder_type)
        self.vision_image_size = int(vision_image_size or expected_image_size)
        self.enable_dinov3_teacher = bool(enable_dinov3_teacher)
        if self.vision_encoder_type not in {"dinov3", "sdtv3_19m"}:
    ...
    def _process_image_pair(self, img_np):
        ...
        if self.vision_image_size != self.expected_image_size:
            img = img.resize((self.vision_image_size, self.vision_image_size), Image.Resampling.BICUBIC)

        dino_pixel_values = self.dino_processor(images=img, return_tensors="pt")["pixel_values"].squeeze(0)
        if tuple(dino_pixel_values.shape[-2:]) != (self.vision_image_size, self.vision_image_size):
            raise RuntimeError(
                f"DINOv3 preprocessor changed the configured vision size; got {tuple(dino_pixel_values.shape[-2:])}"
            )

        if self.vision_encoder_type == "sdtv3_19m":
            student_pixel_values = vision_f.pil_to_tensor(img).float().div_(255.0)
            student_pixel_values = vision_f.normalize(
                student_pixel_values,
                mean=(0.485, 0.456, 0.406),
                std=(0.229, 0.224, 0.225),
            )
        else:
            student_pixel_values = dino_pixel_values

        result = {"pixel_values": student_pixel_values}
        if self.enable_dinov3_teacher:
            # Teacher input intentionally uses the student resolution: the feature
            # distillation loss in trainer.py requires student/teacher token shapes
            # to match exactly (196 tokens/view at 224px vs 256 at native 256px).
            # DINOv3 interpolates its position embedding, so this stays valid.
            result["teacher_pixel_values"] = dino_pixel_values
        return result
...
def vla_collate_fn(batch):
    ...
    for images, instruction, state, action_chunk, action_chunk_mask in batch:
        ...
        img1, img2 = images
        if "pixel_values" not in img1 or "pixel_values" not in img2:
            raise ValueError("Each view must contain preprocessed tensor for key 'pixel_values'")

        image1_list.append(img1["pixel_values"])
        image2_list.append(img2["pixel_values"])
        if "teacher_pixel_values" in img1 or "teacher_pixel_values" in img2:
            if "teacher_pixel_values" not in img1 or "teacher_pixel_values" not in img2:
                raise ValueError("teacher pixel values must be present for both views")
            teacher_image1_list.append(img1["teacher_pixel_values"])
            teacher_image2_list.append(img2["teacher_pixel_values"])
    ...
    samples = {
        "pixel_values": torch.stack(
            [torch.stack(image1_list, dim=0), torch.stack(image2_list, dim=0)], dim=1
        )
    }
    if teacher_image1_list:
        samples["teacher_pixel_values"] = torch.stack(
            [torch.stack(teacher_image1_list, dim=0), torch.stack(teacher_image2_list, dim=0)], dim=1
        )

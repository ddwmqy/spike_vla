...
from ..text.sootspike import load_sootspike_backbone, reset_sootspike_state
from ..text.spikinglm import load_spikinglm_backbone, reset_spiking_state
...
class TurboVLATextEncoder(nn.Module):
    """Selectable ANN BERT or SpikingLM encoder with a shared token interface."""
    ...
    def __init__(self, config: TextEncoderConfig, hidden_dim: int) -> None:
        ...
        if config.encoder_type == "bert":
        ...
        elif config.encoder_type == "spikinglm":
            self.bert = load_spikinglm_backbone(
                checkpoint_path=config.model_name_or_path,
                source_path=str(config.model_source_path),
                time_steps=config.time_steps,
                local_files_only=config.local_files_only,
            )
        elif config.encoder_type == "sootspike":
            self.bert = load_sootspike_backbone(
                checkpoint_path=config.model_name_or_path,
                source_path=str(config.model_source_path),
                time_steps=config.time_steps,
                local_files_only=config.local_files_only,
            ...
            raise ValueError(f"unsupported text encoder type: {config.encoder_type}")
    ...
    def _reset_spike_state(self) -> None:
        """Reset internal SNN state between forwards (no-op for ANN BERT)."""
        if self.config.encoder_type == "spikinglm":
            reset_spiking_state(self.bert)
        elif self.config.encoder_type == "sootspike":
            reset_sootspike_state(self.bert)
    ...
    def encode_bert_hidden(self, instructions: Sequence[str], device: torch.device):
        ...
        for group_length, indices in grouped_indices.items():
            ...
            if hidden is None:
                if group_hidden.ndim == 4:
                    hidden = group_hidden.new_zeros(
                        (group_hidden.shape[0], len(normalized), output_length, group_hidden.shape[-1])
                    )
                else:
    ...
    def forward(self, instructions: Sequence[str], device: torch.device):
        ...
        projection_parameter = next(self.text_projection.parameters())
        hidden = hidden.to(dtype=projection_parameter.dtype)
        text_tokens = self.text_projection(hidden)
        text_key_padding_mask = ~text_token_mask
        if self.config.zero_padded_tokens:
            mask = text_key_padding_mask.unsqueeze(-1)
            if text_tokens.ndim == 4:
                mask = mask.unsqueeze(0)
            text_tokens = text_tokens.masked_fill(mask, 0.0)

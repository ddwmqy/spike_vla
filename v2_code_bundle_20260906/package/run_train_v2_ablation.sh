#!/usr/bin/env bash
# 消融专用启动器:在 run_train_v2.sh 基础上把"视觉 / 文本 / 融合"三个轴变成环境变量。
# 与 run_train_v2.sh 的差异(全部用 [ABLATION] 标出,共 4 处):
#   1) 新增 VISION_TYPE / DOWNSTREAM_TYPE / VISION_*_PATH 五个可覆盖变量;
#   2) 文本分支新增 `bert`(v3 模型侧本支持,原脚本缺这个 case);
#   3) COMMAND 里 4 个硬编码的 vision_* 改为变量;
#   4) COMMAND 里 --downstream_type spiking 改为变量。
# 其余(KD 教师与权重、优化器、数据、步数、精度、PYTHONPATH、resolved-package 校验)一字未动。
#
# 轴取值:
#   VISION_TYPE     = dinov3 | sdtv3_19m        (视觉)
#   TEXT_ENCODER_TYPE = bert | sootspike | spikinglm  (文本)
#   DOWNSTREAM_TYPE = ann | spiking             (融合)
#
# 四臂(KD 全开,教师=官方 LIBERO ckpt,ACT 固定 ann):
#   A0' dinov3     + bert      + ann
#   A1  sdtv3_19m  + bert      + ann
#   A2  dinov3     + sootspike + ann
#   A3  sdtv3_19m  + sootspike + ann
#   (A4 sdtv3_19m  + sootspike + spiking 已有,勿重跑)
set -Eeuo pipefail

EXPERIMENT_ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
CODE_DIR="${CODE_DIR:-$EXPERIMENT_ROOT/code}"          # [ABLATION] 可覆盖,便于异地干跑
...
# --- [ABLATION] 三个消融轴(默认 = 原脚本行为) --------------------------------
VISION_TYPE="${VISION_TYPE:-sdtv3_19m}"            # dinov3 | sdtv3_19m
DOWNSTREAM_TYPE="${DOWNSTREAM_TYPE:-spiking}"      # ann | spiking
# -----------------------------------------------------------------------------
...
# [ABLATION] 视觉权重路径:默认沿用 SDT-V3;dinov3 轴下由调用方改指/置空。
# 注意用 `-`(而非 `:-`):dinov3 臂需要"显式置空"能生效(DINOv3 分支不用这两个路径)。
VISION_MODEL_PATH="${VISION_MODEL_PATH-$SDTV3_CHECKPOINT}"
VISION_PRETRAINED_PATH="${VISION_PRETRAINED_PATH-$SDTV3_CHECKPOINT}"
VISION_SOURCE_PATH="${VISION_SOURCE_PATH-$SDTV3_SOURCE}"
...
# Text encoder selection: bert (ANN) | spikinglm (v2 default) | sootspike (v3).
...
case "$TEXT_ENCODER_TYPE" in
  # [ABLATION] 新增:ANN BERT 分支(模型侧 configuration.py 允许,不需要 source path)
  bert)
    TEXT_WEIGHTS_PATH="${TEXT_WEIGHTS_PATH:-$BERT_PATH}"
    TEXT_SOURCE_ARG=()
    ;;
  spikinglm)
  ...
  sootspike)
    ...
    ;;
  *) die "TEXT_ENCODER_TYPE must be 'bert', 'spikinglm' or 'sootspike', got: $TEXT_ENCODER_TYPE" ;;
esac
...
if [[ "$TEXT_ENCODER_TYPE" == "bert" ]]; then
  [[ -d "$TEXT_WEIGHTS_PATH" ]] || die "BERT weights directory not found: $TEXT_WEIGHTS_PATH"
elif [[ "$TEXT_ENCODER_TYPE" == "spikinglm" ]]; then
...
# [ABLATION] 视觉资产按轴校验(原脚本无条件要求 SDT-V3 三件套)
for path in "$STATS_PATH" "$TEXT_LAYOUT_PATH" "$DINOV3_PATH"; do
  [[ -e "$path" ]] || die "Required path not found: $path"
done
if [[ "$VISION_TYPE" == "sdtv3_19m" ]]; then
  [[ -f "$VISION_MODEL_PATH" ]] || die "SDT-V3 checkpoint not found: $VISION_MODEL_PATH"
  [[ -f "$VISION_PRETRAINED_PATH" ]] || die "SDT-V3 checkpoint not found: $VISION_PRETRAINED_PATH"
  [[ -f "$VISION_SOURCE_PATH" ]] || die "SDT-V3 source not found: $VISION_SOURCE_PATH"
elif [[ "$VISION_TYPE" != "dinov3" ]]; then
  die "VISION_TYPE must be 'dinov3' or 'sdtv3_19m', got: $VISION_TYPE"
fi
[[ -d "$BERT_PATH" ]] || die "BERT directory not found: $BERT_PATH"
[[ -d "$CODE_DIR/turbovla" ]] || die "Required directory not found: $CODE_DIR/turbovla"
...
# [ABLATION] 允许外部预置 TMPDIR:四臂并行同机跑时各用各的临时目录(默认行为不变)
...
COMMAND=(
  ...
  --vision_encoder_type "$VISION_TYPE"                  # [ABLATION] was: sdtv3_19m
  --vision_model_path "$VISION_MODEL_PATH"              # [ABLATION]
  --vision_pretrained_checkpoint "$VISION_PRETRAINED_PATH"   # [ABLATION]
  --vision_model_source_path "$VISION_SOURCE_PATH"      # [ABLATION]
  --vision_image_size 224
  --vision_output_grid_size 14
  --bert_path "$TEXT_WEIGHTS_PATH"
  --text_encoder_type "$TEXT_ENCODER_TYPE"
  # bert 分支无 source path:空数组 + 空值都不能冒出多余参数
  ${TEXT_SOURCE_ARG[@]+"${TEXT_SOURCE_ARG[@]}"} ${TEXT_SOURCE_PATH:+"$TEXT_SOURCE_PATH"}
  --spikinglm_time_steps "$TEXT_TIME_STEPS"
  ${TEXT_ARGS[@]}
  --downstream_type "$DOWNSTREAM_TYPE"                  # [ABLATION] was: spiking
  --downstream_time_steps 4
  --downstream_lif_tau 2.0
  --downstream_lif_backend "${DOWNSTREAM_LIF_BACKEND:-cupy}"
...
echo "[INFO] arm: vision=$VISION_TYPE | text=$TEXT_ENCODER_TYPE (text: $TEXT_MODE) | fusion=$DOWNSTREAM_TYPE | action=ann(ACT, 固定)"
echo "[INFO] KD teacher: $OFFICIAL_CKPT (feature $DISTILL_FEATURE_WEIGHT / action $DISTILL_ACTION_WEIGHT)"
echo "[INFO] GPUs: $GPU_IDS; effective batch: $TARGET_GLOBAL_BATCH; accumulation: $GRAD_ACCUM_STEPS"
echo "[INFO] checkpoints: $CHECKPOINT_DIR; prefix: $CHECKPOINT_PREFIX"

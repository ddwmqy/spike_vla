...
class SimpleMetric:
    ...
    def compute(self):
        ...
        if self.task_name == "stsb":
            pearson = _pearson(predictions, references)
            spearman = _spearman(predictions, references)
            self.predictions.clear()
            self.references.clear()
            return {"pearson": pearson, "spearmanr": spearman, "combined_score": (pearson + spearman) / 2}
        accuracy = float(np.mean(predictions == references))
        result = {"accuracy": accuracy}
        if self.task_name == "cola":
            result = {"matthews_correlation": _matthews_corrcoef(predictions, references)}
        if self.task_name in {"mrpc", "qqp"}:
            true_positive = int(np.sum((predictions == 1) & (references == 1)))
            false_positive = int(np.sum((predictions == 1) & (references != 1)))
            false_negative = int(np.sum((predictions != 1) & (references == 1)))
            denominator = 2 * true_positive + false_positive + false_negative
            result["f1"] = 0.0 if denominator == 0 else (2 * true_positive) / denominator
            result["accuracy"] = accuracy
        self.predictions.clear()
        self.references.clear()
        return result
...
def _matthews_corrcoef(predictions, references):
    true_positive = int(np.sum((predictions == 1) & (references == 1)))
    true_negative = int(np.sum((predictions == 0) & (references == 0)))
    false_positive = int(np.sum((predictions == 1) & (references == 0)))
    false_negative = int(np.sum((predictions == 0) & (references == 1)))
    ...
    return 0.0 if denominator == 0 else ((true_positive * true_negative) - (false_positive * false_negative)) / denominator


def _pearson(predictions, references):
    if predictions.size < 2 or np.std(predictions) == 0 or np.std(references) == 0:
        return 0.0
    return float(np.corrcoef(predictions, references)[0, 1])
...
def _spearman(predictions, references):
    if predictions.size < 2:
        return 0.0
    return _pearson(_rankdata(predictions), _rankdata(references))


def load_metric(task_name):
    return SimpleMetric(task_name)

...
def _lif_backend(config):
    """Use CuPy on CUDA and keep local CPU smoke tests runnable."""
    backend = str(getattr(config, "spiking_backend", "cupy"))
    if backend == "cupy" and not torch.cuda.is_available():
        return "torch"
    return backend
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None, layer_idx=None):
        ...
        backend = _lif_backend(config)
        self.proj_lif = neuron.LIFNode(tau=2.0, detach_reset=True, backend=backend, step_mode='m')
        self.q_lif = neuron.LIFNode(tau=2.0, detach_reset=True, backend=backend, step_mode='m')
        self.k_lif = neuron.LIFNode(tau=2.0, detach_reset=True, backend=backend, step_mode='m')
        self.v_lif = neuron.LIFNode(tau=2.0, detach_reset=True, backend=backend, step_mode='m')
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.attn_lif = neuron.LIFNode(
            tau=2.0, v_threshold=0.5, detach_reset=True, backend=_lif_backend(config), step_mode='m'
        )
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.mlp1_lif = neuron.LIFNode(tau=2.0, detach_reset=True, backend=_lif_backend(config), step_mode='m')
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.mlp2_lif = neuron.LIFNode(tau=2.0, detach_reset=True, backend=_lif_backend(config), step_mode='m')
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        if getattr(self.config, "return_temporal_hidden", False):
            # Preserve the simulation axis for downstream SNN Fusion.  The
            # default remains the released SpikingLM behavior: time averaging.
            temporal_hidden_states = hidden_states
        else:
            temporal_hidden_states = hidden_states.mean(0)
        ...
        return BaseModelOutputWithPastAndCrossAttentions(
            last_hidden_state=temporal_hidden_states,
            past_key_values=past_key_values,

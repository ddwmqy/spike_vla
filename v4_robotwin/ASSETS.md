...
**验收阈值(暂定,待拍板)**:特征类 max|Δ| ≤10%RMS 记"等价"、10–100% 记"可观测偏移(须声明)"、
>100% 记"劣化";动作类固定输入批关节 MAE(raw 单位)≤0.01、夹爪两口径一致率 =100%。
按此口径:**官方默认组合对 B 的文本路径判"劣化"(253%)**;`fp32 + bf16_autocast` 组合两臂
文本均"等价"、视觉"可观测偏移"、动作达标。**注:以上均在随机动作头上测得,相对比较有效;
阈值须在 2k 短训的部署检查后复核。**

...
**§5.3 进度(2026-09-12,本地实测)**:第 1 步"数值检查 A"已跑完(固定输入批 = 2 个真实
clean50 样本、两臂各 9 格、独立进程),完整数据与阈值建议见 `ASSETS.md`。三条对决策有直接影响
的结论:① **精度敏感度高度不对称**——同口径下 B 的 spike 文本投影偏移 **253%RMS**、C1 仅 5.7%,
即统一精度决策的风险几乎全部落在 B 上;② **视觉精度开关在官方默认组合下被外层 autocast 掩盖**
(§1.2/§5.3 把它当"独立开关"不成立,只有 interaction 关 autocast 时才可观测);③ **flash-attn
不支持 fp32**,故"全模块关 autocast 的 FP32 参考"在官方后端下不可达(矩阵改在 sdpa 上跑,
另设一格 flash 量化后端影响:视觉 55.5% vs 61.9%,与精度效应同量级 → 阈值须按实际后端定)。
第 2 步(固定输入批动作误差)已随矩阵一并产出;第 3 步(2k 短训 + rollout 部署检查)待算力。
**待拍板:统一精度组合与验收阈值**(见 `ASSETS.md` "验收阈值(暂定)" 段)。
...
2. **算力**:✅ **2026-09-12 落定 = 算力服务器 4 张卡**。即官方配置路径:
   `NUM_PROCESSES=4 PER_DEVICE_BATCH_SIZE=48 GRADIENT_ACCUMULATION_STEPS=1`(全局 192,
   两臂零妥协,无 accum 档位差异);C1、B 各 ≈1 天,**串行**(并行需 8 卡;若改 2 卡/臂
   则各需 accum=2,两臂对称即可,但墙钟不省)。**评测(C0-20/C0-100/C1/B)按卡分片**,
   与训练排期错开。A-S 仍按 §4 前置(1 卡半天;本地 MIG 已验证 A 臂流水线健康)。
